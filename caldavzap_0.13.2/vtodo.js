...
function todoToVcalendar(operation, accountUID, inputUID, inputEtag, delUID, deleteMode)
{
	...
	//RFC OPTIONAL
	if(vCalendar.tplM['VTcontentline_CATEGORIES'][repeatHash]!=null && (process_elem=vCalendar.tplM['VTcontentline_CATEGORIES'][repeatHash][0])!=undefined)
	{
		// replace the object and related objects' group names (+ append the related objects after the processed)
		parsed=('\r\n'+process_elem).match(RegExp('\r\n((?:'+vCalendar.re['group']+'\\.)?)','m'));
		if(parsed[1]!='')	// if group is present, replace the object and related objects' group names
	...
	{
		process_elem=vCalendar.tplC['VTcontentline_CATEGORIES'];
		process_elem=process_elem.replace('##:::##group_wd##:::##','');
		process_elem=process_elem.replace('##:::##params_wsc##:::##','');
	}

	if($('#categories_TODO').val()!='')
	{
		process_elem=process_elem.replace('##:::##value##:::##',vcalendarEscapeValue($('#categories_TODO').val()));
		vCalendarText+=process_elem;
	}
...
function fullVcalendarToTodoData(inputEvent, formMode)
{
	...
	for(var it=0;it<todoArray.length;it++)
	{
		...
		if(vcalendar_element!=null)
		{
			...
			title=vcalendarUnescapeValue(parsed[4]);
			vCalendar.tplM['VTcontentline_CATEGORIES'][repeatHash] = new Array();
			vCalendar.tplM['VTcontentline_CATEGORIES'][repeatHash][0]=vCalendar.tplC['VTcontentline_CATEGORIES'];
			vCalendar.tplM['VTcontentline_CATEGORIES'][repeatHash][0]=vCalendar.tplM['VTcontentline_CATEGORIES'][repeatHash][0].replace(/##:::##group_wd##:::##/g, parsed[1]);
			vCalendar.tplM['VTcontentline_CATEGORIES'][repeatHash][0]=vCalendar.tplM['VTcontentline_CATEGORIES'][repeatHash][0].replace(/##:::##params_wsc##:::##/g, parsed[3]);
			vtodo=vtodo.replace(vcalendar_element[0], '\r\n');
			...
			if(parsed[1]!='')
			{
				...
				{
					// append the parameter to its parent
					vCalendar.tplM['VTcontentline_CATEGORIES'][repeatHash][0]+=vcalendar_element_related[0].substr(2);
					// remove the processed parameter
					vtodo=vtodo.replace(vcalendar_element_related[0], '\r\n');
				}
			}
...
//					}
		...
		if(isRepeat &&(realStart || realEnd))
		{
			...
			var lastGenDate=generateTodoRepeatInstances({
				...
				items:new todoItems(start,end, realUntilDate, frequency, interval, realUntil, wkst,  repeatStart, repeatEnd, repeatCount, realRepeatCount, byDay, location, categories,  note, title, inputEvent.uid, vcalendar, inputEvent.etag, alertTime, alertNote, status, filterStatus, rec_id, repeatHash,  percent, inputEvent.displayValue, rid, compareString, tzName, realStart, realEnd, alertTimeOut,classType,url,completedOn, toIt, priority,renderPriority, finalAString,ruleString)
			});
		}
		else
		{
			...
			var tmpObj=new todoItems(start, end, '', '', '', '', '',  '', '', '', '', '', location, categories, note, title, inputEvent.uid, vcalendar, inputEvent.etag, alertTime, alertNote, status, filterStatus, rec_id, repeatHash,  percent, inputEvent.displayValue, rid, compareString, tzName, realStart, realEnd, alertTimeOut,classType,url,completedOn,toIt, priority,renderPriority, finalAString,'');
