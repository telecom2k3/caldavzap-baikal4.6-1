...
function dataToVcalendar(operation, accountUID, inputUID, inputEtag, delUID,isFormHidden, deleteMode)
{
	...
	//RFC OPTIONAL
	if(vCalendar.tplM['contentline_CATEGORIES']!=null && (process_elem=vCalendar.tplM['contentline_CATEGORIES'][0])!=undefined)
	{
		// replace the object and related objects' group names (+ append the related objects after the processed)
		parsed=('\r\n'+process_elem).match(RegExp('\r\n((?:'+vCalendar.re['group']+'\\.)?)','m'));
		if(parsed[1]!='')	// if group is present, replace the object and related objects' group names
	...
	{
		process_elem=vCalendar.tplC['contentline_CATEGORIES'];
		process_elem=process_elem.replace('##:::##group_wd##:::##','');
		process_elem=process_elem.replace('##:::##params_wsc##:::##','');
	}
	...
	if($('#categories').val()!='')
	{
		process_elem=process_elem.replace('##:::##value##:::##',vcalendarEscapeValue($('#categories').val()));
		vCalendarText+=process_elem;
	}
...
function fullVcalendarToData(inputEvent)
{
	...
	*/
	...
	for(var it=0;it<eventArray.length;it++)
	{
		...
		//CATEGORIES
		vcalendar_element=vevent.match(RegExp('\r\n'+vCalendar.re['contentline_CATEGORIES'], 'mi'));
		if(vcalendar_element!=null)
		{
			parsed=vcalendar_element[0].match(vCalendar.pre['contentline_parse']);
			//note=String(vcalendar_element).split(':')[1];
			title=vcalendarUnescapeValue(parsed[4]);
			vCalendar.tplM['contentline_CATEGORIES'][0]=vCalendar.tplC['contentline_CATEGORIES'];
			vCalendar.tplM['contentline_CATEGORIES'][0]=vCalendar.tplM['contentline_CATEGORIES'][0].replace(/##:::##group_wd##:::##/g, parsed[1]);
			vCalendar.tplM['contentline_CATEGORIES'][0]=vCalendar.tplM['contentline_CATEGORIES'][0].replace(/##:::##params_wsc##:::##/g, parsed[3]);
			vevent=vevent.replace(vcalendar_element[0], '\r\n');
			...
			if(parsed[1]!='')
			{
				...
				{
					// append the parameter to its parent
					vCalendar.tplM['contentline_CATEGORIES'][0]+=vcalendar_element_related[0].substr(2);
					// remove the processed parameter
					vevent=vevent.replace(vcalendar_element_related[0], '\r\n');
				}
			}
...
//						}
			...
			if(vcalendar_element!=null)
			{
				...
				vcalendar_element=vcalendar.match(vCalendar.pre['contentline_CATEGORIES']);
				if(vcalendar_element!=null)
				{
					parsed=vcalendar_element[0].match(vCalendar.pre['contentline_parse']);
					categories=vcalendarUnescapeValue(parsed[4]);
				}
			...
			if(globalVisibleCalDAVCollections.indexOf(rid)!=-1 || isChange || isNew)
			{
				if(isRepeat)
				{
					...
					if(!inputEvent.isDrawn)
					{
						...
						{
							repeatCount++;
							var tmpObj=new items(inputEvent.etag, start, end, title, all, inputEvent.uid, rid, evid, note, inputEvent.displayValue, alertTime, alertNote, realUntilDate, frequency, interval, realUntil, repeatStart, repeatEnd, byMonthDay,repeatCount, realRepeatCount, vcalendar, location, categories, alertTimeOut,tzName, realStart, realEnd, byDay, rec_id,wkst,classType, avail,hrefUrl, compareString,priority,status,ruleString);
					...
					var lastGenDate=generateRepeatInstances({
						...
						items:new items(inputEvent.etag, varDate, varEndDate, title, all, inputEvent.uid, rid, evid, note, inputEvent.displayValue, alertTime, alertNote, realUntilDate, frequency, interval, realUntil, repeatStart, repeatEnd, byMonthDay, repeatCount, realRepeatCount, vcalendar, location, categories, alertTimeOut, tzName, realStart, realEnd, byDay, rec_id,wkst,classType, avail,hrefUrl,compareString,priority,status,ruleString)
					});
				}
				else
				{
					...
					var tmpObj=new items(inputEvent.etag, start, end, title, all, inputEvent.uid, rid, evid, note, inputEvent.displayValue, alertTime, alertNote, '', '', '', '', '', '', '', '', '', vcalendar, location, categories, alertTimeOut, tzName, realStart, realEnd, byDay, rec_id,wkst,classType, avail,hrefUrl,compareString,priority,status,ruleString);
					if(isChange)
					{
						if(needReload)
